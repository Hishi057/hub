...
つまり、ある有界な領域において、内部で(境界も含めた値の中で)最大値を取るならば、それは$u$が定数にならざるを得ないということを意味している。

要は、内部に最大値が存在すると仮定すると、平均値の定理によって周りに"伝播"していくという話である。

ここで、このことを証明する前に必要となる位相空間論の知識を整理しておく。

### 位相空間論の復習

まず、**領域**とは連結な開集合と説明される。**連結**であるとはつまり、集合が"繋がっている"ということである。**有界**な領域とは、端っこが確かに存在するとも言える。つまり、今回の例において$\Omega$は有界な領域であると説明されているが、これは$x^2+y^2 < R$に集合$\Omega$が収まるような実数$R$が存在すると言っている。

次に、二次元のユークリッド空間において$\Omega$の部分集合$E$が**開集合**であることを示す方法について記述する。それはすなわち、中心$(x,y)$半径$\rho>0$の球$B_{\rho}(x,y) \in \Omega$が、任意の点$(x,y) \in E$に対して$\rho$を適切に選ぶことで、$E$の部分集合となることを示せば良い。逆に、**閉集合**であることを示すには$E$の補集合、つまり$\Omega \setminus E$が開集合であることを示せば十分である。

次に、ある領域$A$が**連結**であることの厳密な定義を確認する。
$A$の部分集合のなかで開集合でかつ閉集合であるものは、$A$自身か$\emptyset$に限られるとき、集合$A$が連結であるという。

### 証明

証明の方針は、次の通り。

まず、$\Omega$の部分集合$E := \{(x,y) \in \Omega \,;\, u(x,y) = M\}$が開集合でかつ閉集合であることを示して、仮定より$E=\emptyset$もあり得ないので、$\Omega$が連結であるという事実から$E = \Omega$に限られることを示すだけである。

改めて、$E$を以下のように定義する。つまり、$\Omega$の中で最大値$M$をとる点の集合を$E$とする。

$$
E := \{(x,y) \in \Omega \,;\, u(x,y) = M\}
$$

$u(x,y) = M$を満たす$(x, y) \in \Omega$が存在するという問題文の仮定の下での話なので、$E$は空集合ではない。

**(i) $E$は開集合**

$0 < r \leq \rho$, $B_\rho(x,y) \subset \Omega$であるとする。任意の点$(x,y) \in E$に対して平均値の定理を適用する。

$$
\begin{aligned}
M &= u(x,y) = \frac{1}{2\pi} \int_0^{2\pi} u(x + r\cos\theta, y + r\sin\theta) \, d\theta \\
&\Leftrightarrow \frac{1}{2\pi} \int_0^{2\pi} \underbrace{(M - u(x + r\cos\theta, y + r\sin\theta))}_{\ge 0} \, d\theta = 0
\end{aligned}
$$

ここで、被積分関数は連続で非負関数なので、積分値が$0$となるには、被積分関数が恒等的に$0$をならざるを得ない。したがって

$$
u(x + r\cos\theta, y + r\sin\theta) = M \quad (0 < r \leq \rho)
$$

である。よって、$B_\rho(x,y) \subset E$なので、$E$は開集合である。

**(ii) $E$は開集合**

$u$は連続なので、ある点$(x,y) \in \Omega \setminus E = \{(x,y) \in \Omega \,;\, u(x,y) < M\}$において、任意の$\varepsilon > 0$に対して以下を満たす$\delta > 0$が存在する。$(x',y') \in B_\delta(x,y) \subset \Omega$として

$$
|u(x',y') - u(x,y)| < \varepsilon
$$

$\varepsilon > 0$は任意なので$\varepsilon = M - u(x,y) > 0$とすることで

$$
u(x',y') < \varepsilon + u(x,y) < M
$$

を得る。したがって、$B_\delta(x,y) \subset \Omega \setminus E$なので、$\Omega \setminus E$は開集合すなわち$E$は閉集合であることが示された。

**(iii) $E=\Omega$**

$\Omega$の部分集合$E$は空集合ではなくて、$(i)$と$(ii)$より開集合でかつ閉集合であることが示されたので$\Omega$が連結であることを考えると$E$は$\Omega$自身とならざるを得ない。

よって、領域$\Omega$において、どの点を取っても最初に定義した最大値$M$を取るのだから、これはつまり$u(x,y)$が定数関数であるという事実に他ならない。
